...
class Api{
  ...
  static Future<List<Game>> getMostPlayedGames() async {
    ...
    if (response.statusCode == 200) {
      ...
      final gamePopular = List<PopularGames>.from(jsonResponse.map((json)=> PopularGames.fromJson(json)));
      final List<Game> games = [];

      for (final game in gamePopular.take(30)) {
        final gameId = game.id;
        final gameDetailsResponse = await http.get(Uri.parse('https://store.steampowered.com/api/appdetails?appids=$gameId'));
  ...
  static Future<List<Game>> searchGame(String query) async{
    ...
    final response = await http.get(Uri.parse('https://steamcommunity.com/actions/SearchApps/$query'));

    if (response.statusCode == 200) {
      final jsonResponse = jsonDecode(response.body);
      final List<String> appIds = List<String>.from(jsonResponse.map((game) => game['appid'] as String));
      final List<Game> games = [];
